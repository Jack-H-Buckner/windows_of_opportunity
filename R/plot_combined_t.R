...
df_sens <- read_csv("data/sensitivity_t.csv", show_col_types = FALSE)
...
t_adapt_start <- if (nrow(unins_below) > 0) min(unins_below$t) else t_close

# ── Top panel: utility vs time ──────────────────────────────────────────────

p_top <- ggplot(df_base, aes(x = t)) +
  annotate("rect",
...
p_bl <- ggplot(df_open, aes(x = elasticity_open, y = param_label)) +
  ...
  labs(x = "Elasticity (proportional effect on time)", y = NULL,
...
ggsave("figures/combined_t.png", fig_stacked, dpi = 400, width = 7, height = 7)
cat("Saved figures/combined_t.png\n")
...
ggsave("figures/combined_t_row.png", fig_row, dpi = 400, width = 10.5, height = 4)
cat("Saved figures/combined_t_row.png\n")
